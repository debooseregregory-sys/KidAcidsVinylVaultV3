...
def update_cd_discogs_release(cd_release_id, data):
    """Attach a Discogs release to an existing CD and persist its tracklist."""
    connection = get_connection()
    try:
        ensure_cd_schema(connection)
        release_id = int(cd_release_id)
        connection.execute(
            ...
            (
                ...
                str(data.get("catalog", "") or "").strip(),
                data.get("year"),
                str(data.get("genre", "") or "").strip(),
                str(data.get("discogs", "") or "").strip(),
                str(data.get("discogs_link", "") or "").strip(),
                str(data.get("cover", "") or "").strip(),
                release_id,
            ),
        )
        connection.commit()
    ...
    return save_cd_discogs_tracks(cd_release_id, data.get("release", {}))
