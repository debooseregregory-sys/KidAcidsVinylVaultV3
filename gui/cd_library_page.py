...
class CDLibraryPage(QWidget):
    ...
    def build_ui(self):
        ...
        self.discogs_button = QPushButton("DISCOGS KOPPELEN")
        self.discogs_button.setMinimumHeight(42)
        self.discogs_button.setToolTip("Koppel de geselecteerde CD aan een Discogs Release ID en importeer de tracklist.")
        self.discogs_button.clicked.connect(self.link_selected_discogs)
        action_row.addWidget(self.discogs_button)
    ...
    def link_selected_discogs(self):
        ...
        if not selected:
            QMessageBox.information(
                self,
                "Geen CD geselecteerd",
                "Selecteer eerst de CD die je aan Discogs wilt koppelen.",
            )
            return

        row = selected[0].row()
        id_item = self.table.item(row, 0)
        artist_item = self.table.item(row, 1)
        ...
        release_id, ok = QInputDialog.getText(
            self,
            "Discogs koppelen",
            f"{artist} — {title}\n\nGeef het Discogs Release ID in:",
        )
        if not ok:
            return
        ...
        self.discogs_button.setEnabled(False)
        self.status.setText("Discogs-release ophalen...")

        try:
            data = fetch_release_data(release_id)
            imported_tracks = update_cd_discogs_release(cd_id, data)
            ...
            QMessageBox.information(
                ...
                (
                    ...
                    f"Discogs ID: {data['discogs']}\n"
                    f"Tracks geïmporteerd: {imported_tracks}"
                ),
            )
        except Exception as error:
            self.status.setText("Discogs-koppeling mislukt")
            ...
            self.discogs_button.setEnabled(True)
