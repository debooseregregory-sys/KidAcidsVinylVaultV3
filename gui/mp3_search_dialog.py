...
class MP3SearchDialog(QDialog):
    ...
    def build_ui(self):
        ...
        disk_search_button = QPushButton("🔎 ZOEK IN PAD")
        disk_search_button.setObjectName("fileButton")
        disk_search_button.clicked.connect(self.search_disk)

        path_row.addWidget(self.search_path_edit, 1)
        path_row.addWidget(path_button)
        path_row.addWidget(disk_search_button)
        layout.addLayout(path_row)

        self.info_label = QLabel("Zoeken in MP3-database...")
    ...
    def search_disk(self):
        ...
        # Gebruik os.walk via Path.rglob. Alleen .mp3 wordt bekeken.
        try:
            files = root.rglob("*.mp3")
            for path in files:
                if not path.is_file():
                    continue
        ...
        self.info_label.setText(
            ...
            f"(van maximaal 500 weergegeven)"
        )
        self.status_label.setText(
