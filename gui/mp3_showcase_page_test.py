...
class VinylDeckWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Keep the current tonearm concept: fixed pivot, long mechanical arm, headshell and stylus.
        ...
        path=[]; path.append(pivot); path.append(elbow); path.append(QPointF(stylus.x()-ux*25,stylus.y()-uy*25))
        ...
        shell=QPointF(stylus.x()-ux*40,stylus.y()-uy*40); angle=math.degrees(math.atan2(dy,dx))
        p.save(); p.translate(shell); p.rotate(angle); p.setBrush(QBrush(QColor("#b9bbc1"))); p.setPen(QPen(QColor("#08090b"),3)); p.drawRoundedRect(QRectF(-27,-9,34,18),3,3); p.restore()
        p.setPen(QPen(QColor("#f1f2f4"),1.6)); p.drawLine(QPointF(stylus.x()-ux*6,stylus.y()-uy*6),QPointF(stylus.x()+ux*3,stylus.y()+uy*3))

        # Physical ON/OFF rocker - no strange glyphs and no round icon.
        switch=QRectF(35,h-88,105,38)
        p.setBrush(QBrush(QColor("#08090b"))); p.setPen(QPen(QColor("#4a4c54"),2)); p.drawRoundedRect(switch,7,7)
        ...
        half=switch.width()/2
        p.setBrush(QBrush(QColor("#d84b91") if self.power_on else QColor("#3c3e46"))); p.drawRoundedRect(QRectF(switch.x()+4 if self.power_on else switch.x()+half+1,switch.y()+5,half-5,28),5,5)
        ...
        p.setBrush(QBrush(QColor("#0c0d11"))); p.setPen(QPen(QColor("#30323a"),1)); p.drawRoundedRect(strip,7,7)
        ...
        p.setBrush(QBrush(status)); p.setPen(Qt.PenStyle.NoPen); p.drawEllipse(QPointF(w-46,h-64),4,4)
    ...
    def mousePressEvent(self,event):
        ...
        w,h,*_=self._layout(); pos=event.position(); switch=QRectF(35,h-88,105,38)
        if switch.contains(pos):
            self.set_power(not self.power_on); event.accept(); return
...
class MP3ShowcasePage(QWidget):
    ...
    def __init__(self,parent=None):
        ...
        # Real audio engine. It is hidden but remains alive while this page exists.
    ...
    def build_ui(self):
        ...
        self.cover=QLabel("NO COVER"); self.cover.setFixedSize(300,300); self.cover.setAlignment(Qt.AlignmentFlag.AlignCenter); self.cover.setStyleSheet("background:#0b0b0f;color:#666672;border:1px solid #302b39;border-radius:6px;"); top.addWidget(self.cover,0,Qt.AlignmentFlag.AlignTop)
    ...
    def show_item(self,row):
        ...
        self.artist_label.setText(artist); self.title_label.setText(title)
        meta=[]
    ...
    def load_tracklist(self,current_path):
        ...
        if not linked:
            self.track_list.addItem("Geen gekoppelde VinylVault-track voor dit bestand"); return
    ...
    def _play_path(self,path):
        ...
        # The real player is called directly. The signal remains available for external integration.
