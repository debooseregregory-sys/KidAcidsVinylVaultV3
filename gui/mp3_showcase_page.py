...
class VinylDeckWidget(QWidget):
    ...
    def set_track(self, artist, title):
        self.artist = clean_display_text(artist).strip() or "Onbekende artiest"
        self.title = clean_display_text(title).strip() or "-"
        self.update()
    ...
    def _layout(self):
        ...
        # Move the platter clearly downward while keeping it inside the deck.
        cy = min(h * 0.63, h - r - 95.0)
        return w, h, cx, cy, r
    ...
    def _text(self, p, rect, text, size=9, color=QColor("#8f919a"), weight=QFont.Weight.Bold, align=Qt.AlignmentFlag.AlignLeft):
        ...
        p.drawText(rect, align, clean_display_text(text))
    ...
    def paintEvent(self, event):
        ...
        g.setColorAt(0, QColor("#292b31"))
        g.setColorAt(.45, QColor("#191a1f"))
        g.setColorAt(1, QColor("#101116"))
        p.setBrush(QBrush(g))
        p.setPen(QPen(QColor("#454750"), 2))
        p.drawRoundedRect(deck, 18, 18)
        p.setBrush(QBrush(QColor("#17181d")))
        p.setPen(QPen(QColor("#30323a"), 1))
        p.drawRoundedRect(QRectF(28, 28, w - 56, h - 56), 11, 11)
        ...
        pg.setColorAt(0, QColor("#555861"))
        pg.setColorAt(.62, QColor("#303239"))
        pg.setColorAt(1, QColor("#17181d"))
        ...
        for i in range(60):
            a = math.radians(i * 6 + self.angle * .16)
            rr = r + 5
            p.setPen(QPen(QColor("#a6a8af"), 2.5 if i % 5 == 0 else 1.4))
            p.drawPoint(QPointF(cx + math.cos(a) * rr, cy + math.sin(a) * rr))

        # VINYL
        rg = QRadialGradient(QPointF(cx - r * .35, cy - r * .35), r * 1.05)
        rg.setColorAt(0, QColor("#24262b"))
        rg.setColorAt(.18, QColor("#0d0e11"))
        rg.setColorAt(.78, QColor("#030406"))
        rg.setColorAt(1, QColor("#111217"))
        ...
        for f in (.985, .965, .945, .925, .905, .885, .865, .845, .825, .805,
        ...
        p.drawArc(QRectF(-r * .88, -r * .88, r * 1.76, r * 1.76), 12 * 16, 55 * 16)
        p.restore()

        # LARGE VINYL LABEL
        label_r = min(105.0, r * .45)
        lg = QRadialGradient(QPointF(cx - label_r * .25, cy - label_r * .25), label_r)
        lg.setColorAt(0, QColor("#b94b7d"))
        lg.setColorAt(.72, QColor("#711b45"))
        lg.setColorAt(1, QColor("#45102b"))
        ...
        self._text(
            p,
            QRectF(cx - label_r * .82, cy - label_r * .48, label_r * 1.64, 28),
            "KID ACID",
            14,
            QColor("#f8e9f0"),
            QFont.Weight.Black,
        ...
        self._text(
            p,
            QRectF(cx - label_r * .82, cy + label_r * .30, label_r * 1.64, 22),
            "VINYL VAULT",
            10,
            QColor("#edb2ca"),
            QFont.Weight.Bold,
        ...
        rest = QPointF(cx + r + 48, cy - r * .31)
        play = QPointF(cx + r * .68, cy + r * .10)
        stylus = QPointF(
            rest.x() + (play.x() - rest.x()) * self.arm_progress,
            rest.y() + (play.y() - rest.y()) * self.arm_progress,
        )
        dx, dy = stylus.x() - pivot.x(), stylus.y() - pivot.y()
        length = max(1.0, math.hypot(dx, dy))
        ux, uy = dx / length, dy / length
        perp = QPointF(-uy, ux)
        elbow = QPointF(
            pivot.x() + dx * .55 + perp.x() * 20,
            pivot.y() + dy * .55 + perp.y() * 20,
        )
        path = [pivot, elbow, QPointF(stylus.x() - ux * 25, stylus.y() - uy * 25)]
        p.setPen(QPen(QColor("#07080a"), 18, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin))
        p.drawPolyline(path)
        p.setPen(QPen(QColor("#9fa2aa"), 12, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin))
        p.drawPolyline(path)
        p.setBrush(QBrush(QColor("#3a3c43")))
        ...
        shell = QPointF(stylus.x() - ux * 40, stylus.y() - uy * 40)
        ...
        # Stylus ends at the playing surface.
        p.setPen(QPen(QColor("#07080a"), 4, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        p.drawLine(QPointF(stylus.x() - ux * 8, stylus.y() - uy * 8), stylus)
        ...
        # SQUARE START BUTTON
        start_rect = QRectF(35, h - 88, 105, 42)
        p.setBrush(QBrush(QColor("#08090b")))
        p.setPen(QPen(QColor("#4a4c54"), 2))
        p.drawRoundedRect(start_rect, 7, 7)
        inner = QRectF(start_rect.x() + 4, start_rect.y() + 4, start_rect.width() - 8, start_rect.height() - 8)
        p.setBrush(QBrush(QColor("#25272d")))
        p.setPen(QPen(QColor("#111216"), 1))
        p.drawRoundedRect(inner, 5, 5)
        p.setBrush(QBrush(QColor("#d84b91") if self.power_on else QColor("#3c3e46")))
        p.setPen(Qt.PenStyle.NoPen)
        p.drawRoundedRect(QRectF(start_rect.x() + 7, start_rect.y() + 7, start_rect.width() - 14, start_rect.height() - 14), 4, 4)
        self._text(
            p,
            QRectF(start_rect.x(), start_rect.y() + 7, start_rect.width(), 28),
            "START",
            11,
            QColor("#ffffff") if self.power_on else QColor("#8f919a"),
            QFont.Weight.Black,
            Qt.AlignmentFlag.AlignCenter,
        )

        # TECHNICAL DISPLAY STRIP
        strip = QRectF(155, h - 92, max(260.0, w - 240), 58)
    ...
    def mousePressEvent(self, event):
        ...
        w, h, *_ = self._layout()
        start_rect = QRectF(35, h - 88, 105, 42)
        if start_rect.contains(event.position()):
...
class MP3ShowcasePage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.audio_player = MP3Player(self)
        self.audio_player.hide()
        self.audio_player.play_started.connect(self._audio_started)
        self.audio_player.stopped.connect(self._audio_stopped)
        self.build_ui()
        self.load_files()
    ...
    def build_ui(self):
        ...
        self.list.itemDoubleClicked.connect(lambda _item: self.play_current())
    ...
    def populate_list(self):
        ...
        self.visible_items = [
            ...
            if not q or q in " ".join(clean_display_text(x) for x in row).casefold()
        ]
        self.list.blockSignals(True)
        self.list.clear()
        for row in self.visible_items:
            artist = clean_display_text(row[1]).strip()
            title = clean_display_text(row[2]).strip()
            text = f"{artist} - {title}".strip(" -") or Path(str(row[0])).name
    ...
    def show_item(self, row):
        ...
        artist = clean_display_text(artist).strip() or "Onbekende artiest"
        title = clean_display_text(title).strip() or Path(str(path)).stem
        album = clean_display_text(album).strip()
        genre = clean_display_text(genre).strip()
        release_artist = clean_display_text(release_artist).strip()
        release_title = clean_display_text(release_title).strip()
        ...
        self.release_label.setText(
            f"Release: {(release_artist + ' - ') if release_artist else ''}{release_title}"
            if release_title else "Release: geen gekoppelde release"
        )
        ...
        if release_cover and Path(release_cover).exists():
            ...
            if not pix.isNull():
                self.cover.setPixmap(
                    pix.scaled(300, 300, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
                )
                self.cover.setText("")
                return
        self.cover.clear()
    ...
    def load_tracklist(self, current_path):
        ...
        try:
            ...
            linked = []
        if not linked:
            self.track_list.addItem("Geen gekoppelde VinylVault-track voor dit bestand")
            return
        for position, title, duration, bpm, path in linked:
            position = clean_display_text(position).strip()
            title = clean_display_text(title).strip()
            text = f"{position} - {title}".strip(" -")
            extras = []
            if duration:
                extras.append(clean_display_text(duration))
            if bpm:
